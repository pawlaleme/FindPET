...
<link href='https://unpkg.com/boxicons@2.0.7/css/boxicons.min.css' rel='stylesheet'>
 <meta name="viewport" content="width=device-width, initial-scale=1.0">
  ...
  <body class="bdashboard">
    ...
    <header>
    <?php                    
            $resultado = editarencontradoPorIDDAO($_GET['idpetencontrado']);                          
      ...
      <div class="wrapper">
        <h2>Editar Pet Encontrado: </h2>
        <div class="input-box">
          <form action="../Forms/processaTodosFormularios.php" enctype="multipart/form-data" method="POST">


                <input type="hidden" name="idpetencontrado" value="<?php echo $_GET['idpetencontrado']; ?>">
                <input type="hidden" name="token" value="<?php echo $registro->foto; ?>">
            
                <label for='status'>Status</label>                
                <?php $statuscorreto = $registro->dataencontro;?> 
                <select id="status" name="status">
                    <option value="disp" <?php if($statuscorreto == null){ echo "selected"; } ?>>Buscando Dono</option>
                    <option value="adopt" <?php if($statuscorreto != null){ echo "selected"; } ?>>Dono Encontrado</option>
                </select>
        ...
        <p class='field'>
          ...
          <input type='text' name='cor' title='Cor' placeholder="Cor do Pet" value="<?php echo $registro->cor; ?>"/>
          <span id='valida' class='i i-close'></span>
        </p>
        
        <p class='field'>
          <label for='user'>Descreva o pet encontrado em algumas palavras e deixe informações importantes</label>
          <input type='text' name='desc' title='Description' placeholder="Fale sobre o pet" value="<?php echo $registro->descricao; ?>"/>
          <span id='valida' class='i i-warning'></span>
        </p>
        
        <p class='field'>
          <label for='user'>Cidade</label>
          <input type='text' name='cidade' title='City' placeholder="Cidade onde ele foi encontrado" value="<?php echo $registro->cidade; ?>"/>
          <span id='valida' class='i i-warning'></span>
        </p>
    ...
    <div class="bottom">
      <ul class="icons">
        <p class="">            
          <label>Envie uma foto do pet que você encontrou: </label>
      ...
      <div class="">
        <button type="submit" name="acao" value="editarpetencontrado">Editar</button>
      </div>
    </div>
    </div>
